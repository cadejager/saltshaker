...
| D26 | **The wasted-seats-cap probe-and-relax (objectives spec §7) is deferred from 2a to 2b** — it belongs with the other relaxation handling (global `-s` auto-relax, fail-loud diagnostics), and no example input exercises it (all are feasible). 2a instead ships **honest failure messages** distinguishing infeasible vs cap-too-tight vs budget-expired. | Decided at 2a's final review (2026-07-03) when the plan had rescoped it; keeps all infeasibility/relaxation logic together in 2b. |
