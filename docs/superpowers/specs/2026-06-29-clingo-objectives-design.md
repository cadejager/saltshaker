...
- **Cap retry:** derive `Cap` from data (`max space − 1`, safe on all three examples). An over-tight cap can fail to return a model *without proving UNSAT*; treat "no model within a short probe budget" as cap-too-tight and retry with a relaxed cap (log it). (Auto-relax of the *global `-s`* cap for true infeasibility is 2b.)
